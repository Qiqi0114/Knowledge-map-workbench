...
var typeCategories= {};
var typeColor = {};// 类型配色
var type= ['Disease', 'Symptom', 'Age', 'Sex', 'BodyStructure', 'Check', 'Thing', 'Severity', 'Population', 'Side', 'BetterAndWorse', 'Body', 'Aggravate', 'Imaging', 'Relieve', 'Event', 'Base', 'Finding', 'BodyStructure_test', 'Action', 'Time']; // 分类
var links: { id: string; source: any; target: any; relation: string; name: string; properties: { status: number; lastModified: number; releaseDate: number; }; semantic_type: string; }[] = []; // 关系集
var nodes: any[] = []; // 节点集
...
watch(
    () => nodes,
      () => {
        // 监听节点变化，设置类型标签
        const obj = {}
        nodes.forEach((e: { semantic_type: string; }) => {
...
const initD3 = async() =>{
      ...
      const simulation = d3.forceSimulation()
        ...
        .force('link', d3.forceLink().id((d: { id: any; }) => d.id).distance(200))
        .force('x', d3.forceX())
        .force('y', d3.forceY())
        .on('tick', ticked)
      ...
      const updateObj = Object.assign(svg.node(), {
        update ({ nodes, links }) {
          ...
          const old = new Map(node.data().map((d: { id: any; }) => [d.id, d]))
          nodes = nodes.map((d: { id: unknown; }) => Object.assign(old.get(d.id) || {}, d))
...
/* 
          link
            .on('mouseover', function (d: { id: string; source: { id: string; }; target: { id: string; }; }) {
              d3.selectAll('.single-node').style('opacity', 0.2)
              d3.selectAll('.single-line').style('opacity', 0.2)
              d3.selectAll('.link-text').style('opacity', 0.2)
...
const nodeColor = (node: { semantic_type: any; }) => {
       ...
       * @param {*} node
       */
const addNodesAndLinks = (node: { id: any; }) => {
        // 模拟接口返回节点和关系数据
        nextTick(() => {
          const res = mock
       ...
       * @param {*} objarray
       */
const uniqObjInArray = (objarray: string|any[]) => {
       ...
       * @param {*} node
       */
const deleteNextNodes = (node: { id: any; }) => {
        const relationNode: any[] = []
        const relationList: any[] = []
        const hasRelationList: any[] = []
        ...
        tempNodeList.forEach(function (item: { id: any; notSingle: boolean; }) {
          const hasLine = relationList.findIndex(jtem => jtem.target.id === item.id || jtem.source.id === item.id)
          if (hasLine >= 0) {
            item.notSingle = true
          }
        })
        tempNodeList.forEach(function (item: { notSingle: any; id: string; }) {
          if (!item.notSingle) {
            d3.select('#single-node' + item.id).remove()
          }
        })
        const otherTempNode: any[] = []
        tempNodeList = tempNodeList.map((item: { notSingle: any; }) => {
          if (!item.notSingle) {
       ...
       * @param {*} node
       */
const deleteNodeAndLinks = (node: { id: any; }) => {
        const removeIndex = nodes.findIndex(data => data.id === node.id)
        ...
        d3.selectAll('.single-node').each(function (d: { id: any; }) {
          const temp = d.id
          // 删除当前需要隐藏的节点
          if (temp === clickNode) {
            d3.select('.menu-circle').remove()
        ...
        d3.selectAll('.link-text').each(function (e: { source: any; target: any; }) {
...
const textBreaking = (dom: { append: (arg0: string) => { (): any; new(): any; attr: { (arg0: string, arg1: number): { (): any; new(): any; attr: { (arg0: string, arg1: number): { (): any; new(): any; text: { (arg0: { (): any; (): any; (): any; }): void; new(): any; }; style: { (arg0: string, arg1: number): { (): any; new(): any; style: { (arg0: string, arg1: string): { (): any; new(): any; text: { (arg0: any): void; new(): any; }; }; new(): any; }; }; new(): any; }; }; new(): any; }; }; new(): any; }; }; text: (arg0: string) => void; }, data: { name: any; }, breaking: boolean) => {
      ...
      /**
        ...
        * @param {*} d
        */
/*       function start(d: { active: any; subject: { fx: any; x: any; fy: any; y: any; }; }) {
                if (!d.active) {
            ...
            function drag(d: { subject: { fx: any; fy: any; }; x: any; y: any; }) {
                d.subject.fx = d.x;
                d.subject.fy = d.y;
            }
            function end(d: { active: any; subject: { fx: null; fy: null; }; }) {
                if (!d.active) {
